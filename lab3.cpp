...
class Object
{
    ...
    public:
        ...
        int lengthWay(int count_mov, int mash){
            return count_mov * mash;
        }
        ~Object(){
		delete[] memory;
        }
...
void rando(Object * stek, int count_obj, int count_mov, int mash){ //рандомное перемещение
    ...
    for(obj=0;obj<count_obj;obj++){
        for(i=1;i<count_mov;i++){
            direct = rand()%5;
            if(direct == 0){ //На месте
                stek[obj].move(0, mash, i);
            }
            if(direct == 1 && stek[obj].getCord(1)+mash<1000){ //Вверх
                stek[obj].move(1, mash, i);
            }
            if(direct == 2 && stek[obj].getCord(0)-mash>0){//Влево
                stek[obj].move(2, mash, i);
            }
            if(direct == 3 && stek[obj].getCord(0)+mash<1000){//Вправо
                stek[obj].move(3, mash, i);
            }
            if(direct == 4 && stek[obj].getCord(1)-mash>0){//Вниз
                stek[obj].move(4, mash, i);
            }
...
int find(int id1, int id2, Object * stek, int count_obj, int count_mov, int mash){//найти расстояние
	...
	for(i=0;i<count_obj;i++){
		if(id1 == stek[i].getId()){ //находит координаты указанных объектов
			x1=stek[i].getCord(0);
			y1=stek[i].getCord(1);
			way1 = stek[i].lengthWay(count_mov, mash);
		}
		if(id2 == stek[i].getId()){
			x2=stek[i].getCord(0);
			y2=stek[i].getCord(1);
			way2 = stek[i].lengthWay(count_mov, mash);
		}
	}
